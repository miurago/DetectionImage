...
def main():
    ...
    # アップローダの作成
    uploaded_file = st.file_uploader("Choose a Image...", type="jpg", key="soyanprprkwaii")
    ...
    if uploaded_file is not None:
        ...
        # Load the model
        model = load_model('sonyanprprkawaii.h5')
        ...
        #teachable_machine_classification関数に画像を引き渡してクラスを推論する
        prediction = teachable_machine_classification(image, 'sonyanprprkawaii.h5')        
        #st.caption(f'推論結果：{prediction}番') # 戻り値の確認（デバッグ用）
        ...
        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「相鉄そうにゃん」です！")
        elif classNo == 1:
            st.subheader(f"これは{pred1}％の確率で「Octocat」です！")
